...
struct RunInThread {
  ...
  explicit RunInThread(T &toRun)
      : _toRun(toRun), _thread([this] { _toRun.run(); }) {}
...
class TestNode {
 public:
  ...
  explicit TestNode(const opencmw::majordomo::Context &context,
                    int socket_type = ZMQ_DEALER)
      : _socket(context, socket_type) {}

  bool bind(const opencmw::URI<opencmw::STRICT> &address) {
    return zmq_invoke(zmq_bind, _socket,
                      opencmw::majordomo::toZeroMQEndpoint(address).data())
        .isValid();
  }

  bool connect(const opencmw::URI<opencmw::STRICT> &address,
               std::string_view subscription = "") {
    auto result =
        zmq_invoke(zmq_connect, _socket,
                   opencmw::majordomo::toZeroMQEndpoint(address).data());
  ...
  bool subscribe(std::string_view subscription) {
    ...
    return zmq_invoke(zmq_setsockopt, _socket, ZMQ_SUBSCRIBE,
                      subscription.data(), subscription.size())
        .isValid();
  }

  bool unsubscribe(std::string_view subscription) {
    assert(!subscription.empty());
    return zmq_invoke(zmq_setsockopt, _socket, ZMQ_UNSUBSCRIBE,
                      subscription.data(), subscription.size())
        .isValid();
  }

  bool sendRawFrame(const std::string &data) {
    opencmw::majordomo::MessageFrame f(
        data, opencmw::majordomo::MessageFrame::dynamic_bytes_tag{});
    return f.send(_socket, 0).isValid();  // blocking for simplicity
  }

  std::optional<MessageType> tryReadOne(
      std::chrono::milliseconds timeout = std::chrono::milliseconds(3000)) {
    std::array<zmq_pollitem_t, 1> pollerItems;
    pollerItems[0].socket = _socket.zmq_ptr;
    pollerItems[0].events = ZMQ_POLLIN;

    const auto result = opencmw::majordomo::zmq_invoke(
        zmq_poll, pollerItems.data(), static_cast<int>(pollerItems.size()),
        timeout.count());
  ...
  void send(MessageType &message) { message.send(_socket).assertSuccess(); }
};

inline bool waitUntilServiceAvailable(
    const opencmw::majordomo::Context &context, std::string_view serviceName,
    const opencmw::URI<opencmw::STRICT> &brokerAddress =
        opencmw::majordomo::INTERNAL_ADDRESS_BROKER) {
  ...
  while (std::chrono::system_clock::now() - startTime < timeout) {
    auto request = opencmw::majordomo::MdpMessage::createClientMessage(
        opencmw::majordomo::Command::Get);
    request.setServiceName(
        "mmi.service", opencmw::majordomo::MessageFrame::static_bytes_tag{});
    request.setBody(serviceName,
                    opencmw::majordomo::MessageFrame::dynamic_bytes_tag{});
    client.send(request);
...
constexpr auto static_tag =
    opencmw::majordomo::MessageFrame::static_bytes_tag{};
constexpr auto dynamic_tag =
    opencmw::majordomo::MessageFrame::dynamic_bytes_tag{};
...
class NonCopyableMovableHandler {
 public:
  ...
  NonCopyableMovableHandler &operator=(NonCopyableMovableHandler &&) noexcept =
      default;
