...
inline std::expected<X509_STORE_Ptr, std::string> createCertificateStore(std::string_view x509_ca_bundle) {
    ...
    if (readCertificateBundleFromBuffer(*cert_store.get(), x509_ca_bundle) <= 0) {
        return std::unexpected(std::format("failed to read certificate bundle from buffer:\n#---start---\n{}\n#---end---\n", x509_ca_bundle));
    }
    return cert_store;
}
...
inline std::expected<EVP_PKEY_Ptr, std::string> readServerPrivateKeyFromBuffer(std::string_view x509_private_key) {
    ...
    BIO_free(certBio);
    if (!privateKeyX509) {
        return std::unexpected(std::format("failed to read private key from buffer"));
    }
...
inline std::expected<EVP_PKEY_Ptr, std::string> readServerPrivateKeyFromFile(std::filesystem::path fpath) {
    ...
    BIO *certBio = BIO_new_file(path.data(), "r");
    if (!certBio) {
        return std::unexpected(std::format("failed to read private key from file {}: {}", path, ERR_error_string(ERR_get_error(), nullptr)));
    }
    EVP_PKEY_Ptr privateKeyX509 = EVP_PKEY_Ptr(PEM_read_bio_PrivateKey(certBio, nullptr, nullptr, nullptr), EVP_PKEY_free);
    BIO_free(certBio);
    if (!privateKeyX509) {
        return std::unexpected(std::format("failed to read private key from file: {}", path));
    }
    return privateKeyX509;
}
...
inline std::span<uint8_t> u8span(std::string_view view) {
...
struct TcpSocket {
    ...
    static std::expected<TcpSocket, std::string> create(SSL_Ptr ssl_, int fd_, int flags_ = VerifyPeer) {
        if (fd_ == -1) {
            return std::unexpected(std::format("Invalid socket file descriptor: {}", fd_));
        }
        TcpSocket socket;
        socket.fd    = fd_;
        socket.flags = flags_;
        socket._ssl  = std::move(ssl_);
        int f        = fcntl(socket.fd, F_GETFL, 0);
        assert(f != -1);
        fcntl(socket.fd, F_SETFL, f | O_NONBLOCK);

        int flag = 1;
        setsockopt(socket.fd, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag));

        if (socket._ssl) {
            auto bio = BIO_new_socket(socket.fd, BIO_NOCLOSE);
            if (!bio) {
                return std::unexpected(std::format("Failed to create BIO object: {}", ERR_error_string(ERR_get_error(), nullptr)));
            }
            // SSL will take ownership of the BIO object
            SSL_set_bio(socket._ssl.get(), bio, bio);
        }
        return socket;
    }
    ...
    std::expected<State, std::string> continueHandshake() {
        ...
        case SSLConnectWantsWrite: {
            ...
            if (ret == 0) {
                return std::unexpected(std::format("SSL handshake failed: {}", ERR_error_string(ERR_get_error(), nullptr)));
            }
    ...
    std::expected<void, std::string> prepareConnect(std::string_view host, uint16_t port) {
        ...
        if (_ssl && (flags & VerifyPeer) != 0) {
            ...
            if (auto r = SSL_ctrl(_ssl.get(), SSL_CTRL_SET_TLSEXT_HOSTNAME, TLSEXT_NAMETYPE_host_name, const_cast<char *>(host.data())); r != SSL_TLSEXT_ERR_OK && r != SSL_TLSEXT_ERR_ALERT_WARNING) {
                return std::unexpected(std::format("Failed to set the TLS SNI hostname: {}", ERR_error_string(ERR_get_error(), nullptr)));
            }
    ...
    std::expected<void, std::string> connect() {
        ...
        if (::connect(fd, address.get()->ai_addr, sizeof(sockaddr)) < 0) {
            ...
            return std::unexpected(std::format("Connect failed: {}", strerror(errno)));
        }
    ...
    std::expected<std::optional<TcpSocket>, std::string> accept(SSL_CTX *ssl_ctx, int flags_) {
        ...
        if (client_fd < 0) {
            ...
            return std::unexpected(std::format("Accept failed: {}", strerror(errno)));
        }

        if (!ssl_ctx) {
            auto maybeSocket = TcpSocket::create({ nullptr, SSL_free }, client_fd);
            if (maybeSocket) {
                maybeSocket->_state = Connected;
            }
            return maybeSocket;
        }
        ...
        auto clientSocket = TcpSocket::create(std::move(ssl.value()), client_fd, flags_);
        if (clientSocket) {
...
struct WriteBuffer {
    ...
    bool write(nghttp2_session *session, TcpSocket &socket) {
        ...
        size -= written;
        std::move(buffer.data() + written, buffer.data() + written + size, buffer.data());
        return true;
    }
};
