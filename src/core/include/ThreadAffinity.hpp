...
constexpr size_t THREAD_MAX_NAME_LENGTH = 256;
...
void setProcessName(const std::string_view &processName) {
#ifdef _POSIX_VERSION
    if (const int rc = prctl(PR_SET_NAME, processName.begin(), 0, 0, 0) != 0) {
        throw std::system_error(rc, thread_exception(), fmt::format("getProcessName()", rc));
    }
#endif
}
