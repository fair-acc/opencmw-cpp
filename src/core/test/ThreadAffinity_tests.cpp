...
TEST_CASE("basic thread affinity", "[ThreadAffinity]") {
    ...
    const auto           dummyAction = [&run]() { while (run) { std::this_thread::sleep_for(std::chrono::milliseconds(10)); } };
...
TEST_CASE("[get,set]ThreadName", "[ThreadAffinity]") {
    using namespace opencmw;
...
TEST_CASE("[get,set]ProcessName", "[ThreadAffinity]") {
    using namespace opencmw;
    ...
    std::atomic<bool> run   = true;
    auto              guard = testTimeoutGuard(std::chrono::milliseconds(1000), run);
    std::jthread      testThread([&run]() {
        thread::setProcessName("otherLocalName");
        REQUIRE("otherLocalName" == thread::getThreadName());
        REQUIRE("otherLocalName" != thread::getProcessName(thread::detail::getPid()));
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
        run = false;
         });
}

TEST_CASE("[get,set]ProcessSchedulingParameter", "[ThreadAffinity]") {
...
TEST_CASE("[get,set]ThreadSchedulingParameter", "[ThreadAffinity]") {
    std::atomic<bool> run         = true;
    auto              guard       = testTimeoutGuard(std::chrono::milliseconds(1000), run);
    const auto        dummyAction = [&run]() { while (run) { std::this_thread::sleep_for(std::chrono::milliseconds(10)); } };
    std::jthread      testThread(dummyAction);
