#ifndef OPENCMW_CIRCULARBUFFER_HPP
#define OPENCMW_CIRCULARBUFFER_HPP
...
public:
    ...
    static inline std::pmr::polymorphic_allocator<T> allocator() {
        return std::pmr::polymorphic_allocator<T>(opencmw::buffer::double_mapped_memory_resource::defaultAllocator());
    }
};
...
class CircularBuffer {
    ...
    public:
        ...
        explicit Reader(std::shared_ptr<BufferImpl> buffer) noexcept : _buffer(buffer) {
            opencmw::buffer::detail::addSequences(_buffer->_claimStrategy._readSequences, _buffer->_claimStrategy._publishCursor, {_readIndex});
        ...
        ~Reader() {
            opencmw::buffer::detail::removeSequence(_buffer->_claimStrategy._readSequences, _readIndex);
...
#endif // OPENCMW_CIRCULARBUFFER_HPP
