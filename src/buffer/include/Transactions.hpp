...
namespace settings {
template<typename T>
struct alignas(opencmw::buffer::hardware_destructive_interference_size) node {
...
static_assert(sizeof(node<int>) % opencmw::buffer::hardware_destructive_interference_size == 0, "node size must be cache line aligned");
...
requires(opencmw::is_power2_v<N_HISTORY> &&N_HISTORY > 8) class SettingBase {
    using TimeStamp  = std::chrono::system_clock::time_point;
    using RingBuffer = opencmw::buffer::CircularBuffer<settings::node<U>, N_HISTORY, opencmw::buffer::ProducerType::Multi, opencmw::buffer::BusySpinWaitStrategy>;
    using Sequence   = opencmw::buffer::Sequence;
    //
    constexpr static std::size_t BUFFER_MARGIN                             = 8;
    alignas(opencmw::buffer::hardware_destructive_interference_size) std::shared_ptr<RingBuffer> _ringBuffer = std::make_shared<RingBuffer>();
    alignas(opencmw::buffer::hardware_destructive_interference_size) std::shared_ptr<Sequence> _sequenceHead = std::make_shared<Sequence>();
    alignas(opencmw::buffer::hardware_destructive_interference_size) std::shared_ptr<Sequence> _sequenceTail = std::make_shared<Sequence>(0);
    alignas(opencmw::buffer::hardware_destructive_interference_size) mutable ReaderWriterLock _historyLock{};
    alignas(opencmw::buffer::hardware_destructive_interference_size) mutable std::atomic<bool> _transactionListLock{ false };
    std::list<std::pair<TransactionToken, settings::node<T>>> _transactionList;
    alignas(opencmw::buffer::hardware_destructive_interference_size) const std::function<U(const U &, T &&)> _onCommit;
