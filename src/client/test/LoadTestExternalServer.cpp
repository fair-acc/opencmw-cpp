...
TEST_CASE("Load test with external server", "[majordomo][majordomoworker][load_test][http2]") {
    ...
    constexpr auto                                             kNUpdates     = 5UZ;
    constexpr auto                                             kIntervalMs   = 200UZ;
    ...
    waitFor(responseCount, kNUpdates * clients.size(), std::chrono::seconds(15));
    ...
    // TODO(Frank) maybe print more detailed distribution
    ...
    // TODO(Frank) compute drift over time
}
