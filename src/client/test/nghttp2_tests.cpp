...
TEST_CASE("Basic Client Constructor and API Tests", "[http2]") {
    ...
    RestClient client2(DefaultContentTypeHeader(MIME::HTML), ClientCertificates(testCertificate));
...
TEST_CASE("GET HTTP", "[http2]") {
    ...
    req0.topic           = URI<>(fmt::format("http://localhost:{}/sayhello?client=http", kServerPort));
    ...
    req1.topic           = URI<>(fmt::format("https://localhost:{}/sayhello?client=https", kServerPort));
...
TEST_CASE("HTTPS", "[http2]") {
    ...
    auto        serverThread = std::jthread([&](std::stop_token stopToken) {
        ...
        if (!server) {
            FAIL(fmt::format("Failed to create server: {}", server.error()));
            return;
        }
    ...
    client::Command  req0;
    req0.command  = mdp::Command::Get;
    req0.topic           = URI<>(fmt::format("https://localhost:{}/sayhello?client=doesntCare", kServerPort));
    req0.clientRequestID = opencmw::IoBuffer("0");
    req0.callback = [&responseCount](const mdp::Message &msg) {
        REQUIRE(msg.command == mdp::Command::Final);
        REQUIRE(msg.protocolName == "https");
    ...
    client::Command req1;
    req1.command         = mdp::Command::Get;
    req1.topic           = URI<>(fmt::format("https://localhost:{}/sayhello?client=trusting", kServerPort));
    req1.clientRequestID = opencmw::IoBuffer("0");
    req1.callback        = [&responseCount](const mdp::Message &msg) {
        REQUIRE(msg.command == mdp::Command::Final);
        REQUIRE(msg.protocolName == "https");
    ...
    req2.topic           = URI<>(fmt::format("https://localhost:{}/sayhello?client=notTrusting", kServerPort));
...
TEST_CASE("GET/SET", "[http2]") {
    ...
    req0.command         = mdp::Command::Get;
    req0.clientRequestID = opencmw::IoBuffer("0");
    req0.topic           = URI<>(fmt::format("http://localhost:{}/foo?param1=1&param2=foo%2fbar", kServerPort));
    req0.callback        = [&responseCount](const mdp::Message &msg) {
        REQUIRE(msg.command == mdp::Command::Final);
    ...
    req1.topic           = URI<>(fmt::format("http://localhost:{}/bar?param1=1&param2=2", kServerPort));
    req1.callback        = [&responseCount](const mdp::Message &msg) {
        REQUIRE(msg.command == mdp::Command::Final);
        REQUIRE(msg.protocolName == "http");
    ...
    req2.topic           = URI<>(fmt::format("http://localhost:{}/setexample", kServerPort));
...
TEST_CASE("Long polling example", "[http2]") {
    ...
    auto          brokerThread = std::jthread([](std::stop_token stopToken) {
        ...
        ensureMessageReceived(server, stopToken, messages, std::chrono::seconds(10)); // makes sure requests are processed
    });
...

    {
        ...
        sub.topic    = URI<>(fmt::format("http://localhost:{}/foo?param1=1&param2=foo%2fbar", kServerPort));
