 ...
 */
        ...
        std::array<std::string, 4> getPreferredContentTypeHeader(const URI<STRICT> &uri, auto _mimeType) {
            ...
            if (const auto acceptHeader = uri.queryParamMap().find(ACCEPT_HEADER); acceptHeader != uri.queryParamMap().end() &&
                                                                                   acceptHeader->second) {
        ...
        struct FetchPayload {
            ...
            void returnMdpMessage(unsigned short status, std::string_view body, std::string_view errorMsgExt = "") noexcept {
                ...
                try {
                    ...
                    std::cerr
                            << fmt::format("caught exception '{}' in FetchPayload::returnMdpMessage(cmd={}, {}: {})", e.what(), command.topic, status,
                                           body) << std::endl;
                } catch (...) {
                    std::cerr
                            << fmt::format("caught unknown exception in FetchPayload::returnMdpMessage(cmd={}, {}: {})", command.topic, status, body)
                            << std::endl;
                }
            }
        ...
        struct SubscriptionPayload : FetchPayload {
            ...
            void requestNext() {
                auto uri = opencmw::URI<opencmw::STRICT>::UriFactory(command.topic).addQueryParameter("LongPollingIdx",
                                                                                                      (_update == 0) ? "Next" : fmt::format("{}",
                                                                                                                                            _update)).build();
                ...
                static auto getPayloadIt = [](emscripten_fetch_t *fetch) {
                    ...
                    if (it == detail::subscriptionPayloads.end()) {
                        fmt::print("RestClientEmscripten::payloadError: url: {}, bytes: {}\n", fetch->url, fetch->numBytes);
...
private:
    void executeCommand(Command &&cmd) const {
        ...
        attr.requestHeaders = preferredHeaderEmscripten.data();
        attr.onsuccess      = [](emscripten_fetch_t *fetch) {
            //fmt::print("RestClientEmscripten: got get/set reply: {}\n", fetch->url);
    ...
    void stopSubscription(Command &&cmd) {
        ...
        fmt::print("stopping subscription: {}, existing subscriptions: {}\n", cmd.topic.str(), detail::subscriptionPayloads.size());
