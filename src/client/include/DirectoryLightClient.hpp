...
auto parse = [](const std::string &reply) {
    ...
    for (auto l : lines | split_lines) {
        ...
        if (l2.empty()) { devices.insert({devicename, {}}); continue; }
        std::string classname{std::string_view{l2.front().data(), l2.front().size()}};
        if (classname.starts_with("*NOT_BOUND*") || classname.starts_with("*UNKNOWN*")) { devices.insert({devicename, {}}); continue; }
        auto l3 = std::views::drop(l2, 1);
        if (l3.empty()) { devices.insert({devicename, {}}); continue; }
