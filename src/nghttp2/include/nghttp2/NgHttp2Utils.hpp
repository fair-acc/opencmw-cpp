...
struct TcpSocket {
    ...
    static std::expected<TcpSocket, std::string> create(SSL_Ptr ssl_, int fd_, int flags_ = VerifyPeer) {
        if (fd_ == -1) {
            return std::unexpected(fmt::format("Invalid socket file descriptor: {}", fd_));
        }
        TcpSocket socket;
        socket.fd    = fd_;
        socket.flags = flags_;
        ...
        if (socket._ssl) {
            ...
            if (!bio) {
                return std::unexpected(fmt::format("Failed to create BIO object: {}", ERR_error_string(ERR_get_error(), nullptr)));
            }
