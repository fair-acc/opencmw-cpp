    ...
    static void deserialise(IoBuffer &buffer, FieldDescription auto const &field, T &value) {
        ...
        if constexpr (is_supported_number<K> || is_stringlike<K>) {
            if (yas::getDataTypeId<K>() != keyType) {
                throw new ProtocolException(fmt::format("key type mismatch for field {} - required {} ({}) vs. have {}", field.fieldName, yas::getDataTypeId<K>(), typeid(K).name(), keyType));
            }
            const auto nElementsCheck = static_cast<uint32_t>(buffer.get<int32_t>());
            if (nElements != nElementsCheck) {
                throw new ProtocolException(fmt::format("key array length mismatch for field {} - required {} vs. have {}", field.fieldName, nElements, nElementsCheck));
            }
            for (auto i = 0U; i < nElementsCheck; i++) {
                keys.emplace_back(buffer.get<K>());
            }
        } else if (keyType == yas::getDataTypeId<OTHER>()) {
            throw new ProtocolException(fmt::format("key type OTHER for field {} not yet implemented", field.fieldName));
        } else {
            throw new ProtocolException(fmt::format("unsupported key type {} for field {}", keyType, field.fieldName));
        }

        const auto valueType = buffer.get<uint8_t>();
        if constexpr (is_supported_number<V> || is_stringlike<V>) {
            if (yas::getDataTypeId<K>() != keyType) {
                throw new ProtocolException(fmt::format("value type mismatch for field {} - required {} ({}) vs. have {}", field.fieldName, yas::getDataTypeId<V>(), typeid(V).name(), valueType));
            }
            const auto nElementsCheck = static_cast<uint32_t>(buffer.get<int32_t>());
            if (nElements != nElementsCheck) {
                throw new ProtocolException(fmt::format("value array length mismatch for field {} - required {} vs. have {}", field.fieldName, nElements, nElementsCheck));
            }
            value.clear();
            for (auto i = 0U; i < nElementsCheck; i++) {
                auto v         = buffer.get<V>();
            ...
            throw new ProtocolException(fmt::format("value type OTHER for field {} not yet implemented", field.fieldName));
        } else {
            throw new ProtocolException(fmt::format("unsupported key type {} for field {}", keyType, field.fieldName));
        }
    }
