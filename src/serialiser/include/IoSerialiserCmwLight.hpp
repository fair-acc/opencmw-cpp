...
template<typename T>
void skipRawArray(IoBuffer &buffer) {
...
void skipArray(IoBuffer &buffer) {
    buffer.skip(2 * static_cast<int32_t>(sizeof(int32_t)));
    skipRawArray<T>(buffer);
}

inline void skipString(IoBuffer &buffer) { skipRawArray<char>(buffer); }

template<typename T>
void skipMultiArray(IoBuffer &buffer) {
    buffer.skip(buffer.get<int32_t>() * static_cast<int32_t>(sizeof(int32_t))); // skip size header
    skipRawArray<T>(buffer);                                                      // skip elements
}
...
struct IoSerialiser<CmwLight, T> {
    static constexpr uint8_t getDataTypeId() {
        ...
        // clang-format on
        return 0xff; // never reached but suppresses control flow warnings
    }
...
struct IoSerialiser<CmwLight, T> {
    static constexpr uint8_t getDataTypeId() { return 7; }
...
struct IoSerialiser<CmwLight, T> {
    ...
    constexpr static void deserialise(IoBuffer &buffer, FieldDescription auto const & /*field*/, T &value) noexcept {
        const int nx = buffer.get<int32_t>();
        const int ny = buffer.get<int32_t>();
        buffer.getArray(value, nx * ny);
    }
};
...
struct IoSerialiser<CmwLight, START_MARKER> {
    static constexpr uint8_t getDataTypeId() { return 0x08; }
...
struct IoSerialiser<CmwLight, END_MARKER> {
    static constexpr uint8_t getDataTypeId() { return 0xFE; }
...
struct IoSerialiser<CmwLight, OTHER> {
    static constexpr uint8_t getDataTypeId() { return 0xFD; }
    ...
    static void deserialise(IoBuffer &buffer, FieldDescription auto const &field, const OTHER &) {
        ...
        // arrays
        else if (typeId == getTypeIdVector<bool       >()) { skipArray<bool       >(buffer); }
        ...
        // 2D and Multi array
        else if (typeId == getTypeIdMultiArray<bool       , 2>() || typeId == getTypeIdMultiArray<bool       , 3>()) { skipMultiArray<bool       >(buffer); }
...
struct FieldHeaderReader<CmwLight> {
    ...
    static void get(IoBuffer &buffer, DeserialiserInfo & /*info*/, FieldDescription auto &field) {
 ...
 * Therefore, for now only serialisation is implemented, and even there we have to retroactively overwrite the field header's type ID from within the
 * serialise function.
 */
...
struct IoSerialiser<CmwLight, std::variant<Types...>> {
    ...
    constexpr static void serialise(IoBuffer &buffer, FieldDescription auto const &field, const std::variant<Types...> &value) noexcept {
        std::visit([&buffer, &field]<typename T>(T &val) {
            ...
            // overwrite the field header with the actual type
    ...
    constexpr static void deserialise(IoBuffer & buffer, FieldDescription auto const & parent, std::variant<Types...> & value) {
        auto deserialiseVariant = [&]<typename T>() {
            if (parent.intDataType == IoSerialiser<CmwLight, T>::getDataTypeId()) {
                T result;
                IoSerialiser<CmwLight, T>::deserialise(buffer, parent, result);
                value = result;
                return true;
            }
            return false;
        };
        if (!(deserialiseVariant.template operator()<Types>() || ...)) {
            throw ProtocolException(std::format("Type in data is not supported by the requested variant: {}", parent.intDataType));
        }
    }
};
    ...
    constexpr static void serialise(IoBuffer &buffer, FieldDescription auto const &parentField, const std::map<std::string, ValueType> &value) noexcept {
        ...
        for (auto &[key, val] : value) {
            if constexpr (isReflectableClass<ValueType>()) { // nested data-structure
                ...
                [[maybe_unused]] const std::size_t     posSizePositionStart = FieldHeaderWriter<CmwLight>::put<true>(buffer, field, val);
                [[maybe_unused]] const std::size_t     posStartDataStart    = buffer.size();
                return;
            } else { // field is a (possibly annotated) primitive type
                FieldDescription auto field = detail::newFieldHeader<CmwLight, true>(buffer, key.c_str(), parentField.hierarchyDepth + 1, val, 0);
                FieldHeaderWriter<CmwLight>::put<true>(buffer, field, val);
            }
        }
    }

    constexpr static void deserialise(IoBuffer &buffer, FieldDescription auto &parent, std::map<std::string, ValueType> &value) {
        using protocol         = CmwLight;
        constexpr auto   check = ProtocolCheck::IGNORE;
        IoSerialiser<protocol, START_MARKER>::deserialise(buffer, parent, START_MARKER_INST);
        auto             field = detail::newFieldHeader<CmwLight, true>(buffer, "", parent.hierarchyDepth, ValueType{}, parent.subfields);
        while (buffer.position() < buffer.size()) {
            DeserialiserInfo info;
            auto previousSubFields = field.subfields;
            FieldHeaderReader<protocol>::get<check>(buffer, info, field);
            ...
            if (field.intDataType == IoSerialiser<protocol, END_MARKER>::getDataTypeId()) { // reached end of sub-structure
                try {
                    ...
                    if (detail::handleDeserialisationErrorAndSkipToNextField<check>(buffer, field, info, "IoSerialiser<{}, END_MARKER>::deserialise(buffer, {}::{}, END_MARKER_INST): position {} vs. size {} -- exception: {}",
            ...
            if constexpr (isReflectableClass<ValueType>()) {
                ...
                buffer.set_position(field.headerStart); // reset the buffer position for the nested deserialiser to read again
                ++field.hierarchyDepth;
                ...
                if (requestedType != field.intDataType && requestedType != IoSerialiser<protocol, OTHER>::getDataTypeId()) { // mismatching data-type
                    detail::moveToFieldEndBufferPosition(buffer, field);
                    detail::handleDeserialisationError<check>(info, "mismatched field type for map field {} - requested type: {} (typeID: {}) got: {}", field.fieldName, typeName<ValueType>, requestedType, field.intDataType);
                    continue;
                }
