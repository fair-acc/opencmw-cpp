...
TEST_CASE("IoSerialiser basic syntax", "[IoSerialiser]") {
    opencmw::debug::resetStats();
    {
        ...
        opencmw::FieldHeaderWriter<opencmw::YaS>::template put<true>(buffer, "fieldNameA", strlen("fieldNameA"), std::move(43.0));
        opencmw::FieldHeaderWriter<opencmw::YaS>::template put<true>(buffer, "fieldNameB", strlen("fieldNameB"), data.value.value());
        std::cout << fmt::format("buffer size (after): {} bytes\n", buffer.size());
    }
    REQUIRE(opencmw::debug::dealloc == opencmw::debug::alloc); // a memory leak occurred
    opencmw::debug::resetStats();
