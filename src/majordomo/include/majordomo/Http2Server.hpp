...
struct Http3Session : public SessionBase<Http3Session<TServer>, std::int64_t>, public std::enable_shared_from_this<Http3Session<TServer>> {
    ...
    int init(const Endpoint &ep, const Address &local_addr,
        ...
        auto callbacks = ngtcp2_callbacks{
            ...
            return 0; },
            .update_key = [](ngtcp2_conn *conn, uint8_t *rx_secret, uint8_t *tx_secret, ngtcp2_crypto_aead_ctx *rx_aead_ctx, uint8_t *rx_iv, ngtcp2_crypto_aead_ctx *tx_aead_ctx, uint8_t *tx_iv, const uint8_t *current_rx_secret, const uint8_t *current_tx_secret, size_t secretlen, void *user_data) {
            auto session = static_cast<Http3Session *>(user_data);
            ...
            return 0; },
            .path_validation = [](ngtcp2_conn *conn, uint32_t flags, const ngtcp2_path *path, const ngtcp2_path *old_path, ngtcp2_path_validation_result res, void *user_data) {
                ...
                return 0; },
            .stream_reset = [](ngtcp2_conn *conn, int64_t stream_id, uint64_t final_size, uint64_t app_error_code, void *user_data, void *stream_user_data) {
            auto session = static_cast<Http3Session *>(user_data);
            if (session->on_stream_reset(stream_id) != 0) {
                return NGTCP2_ERR_CALLBACK_FAILURE;
            }
            return 0; },
            .extend_max_remote_streams_bidi = [](ngtcp2_conn *conn, uint64_t max_streams, void *user_data) {
            auto session = static_cast<Http3Session *>(user_data);
            session->extend_max_remote_streams_bidi(max_streams);
            return 0; },

            .extend_max_stream_data = [](ngtcp2_conn *conn, int64_t stream_id, uint64_t max_data, void *user_data, void *stream_user_data) {
                auto session = static_cast<Http3Session *>(user_data);
            ...
            .stream_stop_sending = [](ngtcp2_conn *conn, int64_t stream_id, uint64_t app_error_code, void *user_data, void *stream_user_data) {
            auto session = static_cast<Http3Session *>(user_data);
    ...
    int handshake_completed() {
        ...
        auto tokenlen = ngtcp2_crypto_generate_regular_token(token.data(), this->_sharedData->_static_secret.data(), this->_sharedData->_static_secret.size(), path->remote.addr, path->remote.addrlen, t);
        if (tokenlen < 0) {
            HTTP_DBG("Server::H3::handshake_completed: ngtcp2_crypto_generate_regular_token failed");
            return 0;
        }

        if (auto rv = ngtcp2_conn_submit_new_token(_conn, token.data(), tokenlen); rv != 0) {
    ...
    int on_read(const Endpoint &ep, const Address &local_addr, const sockaddr *sa, socklen_t salen, const ngtcp2_pkt_info *pi, std::span<const uint8_t> data) {
        ...
        if (auto rv = ngtcp2_conn_read_pkt(_conn, &path, pi, data.data(), data.size(), util::timestamp()); rv != 0) {
            switch (rv) {
        ...
        for (;;) {
            ...
            if (_httpconn && ngtcp2_conn_get_max_data_left(_conn)) {
                ...
                if (sveccnt < 0) {
                    ...
                    ngtcp2_ccerr_set_application_error(&_last_error, nghttp3_err_infer_quic_app_error_code(sveccnt), nullptr, 0);
                    return handle_error();
                }
            }
