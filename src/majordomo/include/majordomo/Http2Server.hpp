...
struct Http3Session : public SessionBase<Http3Session<TServer>, std::int64_t>, public std::enable_shared_from_this<Http3Session<TServer>> {
    ...
    int           http_stream_close(std::int64_t stream_id, std::uint64_t error_code) {
    ...
    int http_recv_data(std::int64_t stream_id, std::string_view data) {
        HTTP_DBG("Server::H3::recv_data: stream_id={} datalen={}", stream_id, data.size());
        this->addData(stream_id, data);
        return 0;
    }

    int http_deferred_consume(std::int64_t stream_id, std::size_t datalen) {
    ...
    int setup_httpconn() {
        ...
        callbacks.stream_close = [](nghttp3_conn *, std::int64_t stream_id, std::uint64_t error_code, void *conn_user_data, void *) {
            auto session = static_cast<Http3Session *>(conn_user_data);
            return session->http_stream_close(stream_id, error_code);
        };
...
struct RestServer {
    ...
    int send_retry(const ngtcp2_pkt_hd *chd, Endpoint &ep, const Address &local_addr, const sockaddr *sa, socklen_t salen, size_t max_pktlen) {
        ...
        if (nwrite < 0) {
            HTTP_DBG("ngtcp2_crypto_write_retry: {}", ngtcp2_strerror(nwrite));
            return -1;
        }
