...
struct Session {
    ...
    void handleTimeouts(const TimePoint &now, std::chrono::milliseconds majordomoTimeout) {
        ...
        // In case of multiple requests n, n+1, ..., on timeout of n, we need to timeout n+1, ... as well,
        // to avoid races where the client could receive a response for n+1 before n
        auto pollIt = _pendingPolls.begin();
        std::vector<std::pair<std::int32_t, std::uint64_t>> timedOut;
        while (pollIt != _pendingPolls.end()) {
            const auto &[timestamp, zmqTopic, pollingIndex, streamId] = *pollIt;
            const auto previousTimedOut                               = std::ranges::find(timedOut, std::pair{ streamId, pollingIndex }) != timedOut.end();
            if (now - timestamp > majordomoTimeout || previousTimedOut) {
                timedOut.emplace_back(streamId, pollingIndex);
                pollIt = _pendingPolls.erase(pollIt);
            } else {
                ++pollIt;
            }
        }
        for (const auto &[streamId, pollingIndex] : timedOut | std::views::reverse) {
    ...
    std::optional<Message> processLongPollRequest(std::int32_t streamId, const Request &request) {
        ...
        if (request.longPollIndex == "Next") {
            ...
            respondWithLongPollingRedirect(streamId, request.topic.toMdpTopic(), entry.firstIndex);
            return result;
        ...
        if (auto [ptr, ec] = std::from_chars(request.longPollIndex.data(), request.longPollIndex.data() + request.longPollIndex.size(), index); ec == std::errc()) {
            if (index < entry.firstIndex) {
                ...
                _pendingPolls.emplace_back(now, zmqTopic, index, streamId);
            } else {
...
struct Http2Server {
    ...
    void handleTimeouts() {
        ...
        for (auto &session : _sessions | std::views::values) {
            session->handleTimeouts(now, _majordomoTimeout);
        }
    }
    ...
    std::vector<Message> processReadWrite(int fd, bool read, bool write) {
        ...
        uint8_t buffer[1024];
        ssize_t bytes_read = session->_socket.read(buffer, sizeof(buffer));
        ...
        if (nghttp2_session_mem_recv2(session->_session, buffer, static_cast<size_t>(bytes_read)) < 0) {
