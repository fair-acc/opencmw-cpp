...
TEST_CASE("Load test", "[majordomo][majordomoworker][load_test][http2]") {
    ...
    for (std::size_t i = 0; i < clients.size(); i++) {
        for (std::size_t j = 0; j < kSubscriptions; j++) {
            ...
            cmd.topic        = URI<>(fmt::format("http://localhost:{}/loadTest?topic={}&intervalMs={}&payloadSize={}&nUpdates={}&initialDelayMs={}", kServerPort, topic, kIntervalMs, kPayloadSize, kNUpdates, kInitialDelayMs));
    ...
    waitFor(responseCount, kNUpdates * kSubscriptions * kNClients, 10s);

    fmt::println("Received {} responses in {}ms (Net production time: {}ms)", responseCount.load(), std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - start).count(), kInitialDelayMs + (kNUpdates * kIntervalMs));
    ...
    fmt::println("Average latency: {}µs", averageLatency / 1000.0);
    fmt::println("{}", fmt::join(latenciesOfi0j0, ", "));
