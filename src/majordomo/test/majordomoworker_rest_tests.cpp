...
public:
    ...
    explicit ColorWorker(const BrokerType &broker, std::vector<ColorContext> notificationContexts)
        ...
        super_t::setCallback([](majordomo::RequestContext & /*rawCtx*/, const ColorContext &inCtx, const majordomo::Empty &, ColorContext &outCtx, SingleString &out) {
            outCtx    = inCtx;
            out.value = fmt::format("red={}, green={}, blue={}\n", inCtx.red, inCtx.green, inCtx.blue);
            FAIL(fmt::format("Unexpected GET/SET request: {}", out.value));
        });
...
TEST_CASE("Simple REST example", "[majordomo][majordomoworker][simple_example][http2]") {
    ...
    serviceList.topic    = opencmw::URI<>(fmt::format("http://localhost:{}", kServerPort));
    ...
    serviceList2.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/", kServerPort));
    ...
    getAsset.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/assets/main.css", kServerPort));
    ...
    getAsset404.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/assets/does-not-exist", kServerPort));
    ...
    // TODO(Frank) old code used contentType "application/javascript"
    getJson.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/addressbook?ctx=FAIR.SELECTOR.ALL&contentType=application%2Fjson", kServerPort));
    ...
    getHtml.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/addressbook?ctx=FAIR.SELECTOR.ALL&contentType=text%2Fhtml", kServerPort));
    ...
    postJson.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/addressbook?ctx=FAIR.SELECTOR.ALL&contentType=application%2Fjson", kServerPort));
    ...
    getJsonAfterSet.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/addressbook?ctx=FAIR.SELECTOR.ALL&contentType=application%2Fjson", kServerPort));
...
TEST_CASE("Invalid paths", "[majordomo][majordomoworker][rest]") {
    ...
    getSpace.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/paths/with%20space", kServerPort));
    ...
    invalidSubscription.topic    = opencmw::URI<>(fmt::format("http://localhost:{}//p-a-t-h-s/?LongPollIdx=Next", kServerPort));
...
TEST_CASE("Get/Set with subpaths", "[majordomo][majordomoworker][rest]") {
    ...
    getEmpty.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/paths", kServerPort));
    ...
    getOne.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/paths/a", kServerPort));
    ...
    getTwo.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/paths/a/b", kServerPort));
...
TEST_CASE("Subscriptions", "[majordomo][majordomoworker][subscription]") {
    ...
    allSub.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors", kServerPort));
    ...
    redSub.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors?red", kServerPort));
    ...
    yellowSub.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors?red&green", kServerPort));
    ...
    whiteSub1.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors?red&green&blue", kServerPort));
    ...
    whiteSub2.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors?green&red&blue", kServerPort));
    ...
    whiteSub3.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/colors?blue&green&red", kServerPort));
...
TEST_CASE("Handler matching", "[majordomo][rest]") {
    ...
    auto echoHandler = [](std::string method, std::string path) {
        return majordomo::rest::Handler{
            ...
            .handler = [path, method](const auto &) {
                ...
                response.code = 200;
                response.body.put<opencmw::IoBuffer::WITHOUT>(fmt::format("{}|{}", method, path));
                return response;
            }
        };
    };
    ...
    getRoot.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/", kServerPort));
    ...
    getAssets.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/assets/test.txt", kServerPort));
    ...
    getSubresource.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/assets/subresource", kServerPort));
    ...
    getSubresource2.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/assets/subresource/extra", kServerPort));
...
TEST_CASE("File system handler", "[majordomo][majordomoworker][rest]") {
    ...
    majordomo::rest::Settings rest;
    rest.port     = kServerPort;
    rest.handlers = { majordomo::rest::fileSystemHandler("/files/*", std::filesystem::current_path()) };
    auto bound    = broker.bindRest(rest);
    if (!bound) {
        FAIL(fmt::format("Failed to bind REST server: {}", bound.error()));
        return;
...
-----END CERTIFICATE-----
    ...
    fileExists.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/files/demo_public.crt", kServerPort));
    ...
    fileDoesNotExist.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/files/does_not_exist", kServerPort));
...
TEST_CASE("Subscription latencies", "[majordomo][majordomoworker][rest]") {
    ...
    std::atomic<int> msLatency = 0;
    {
        ...
        majordomo::rest::Settings                                   rest;
        rest.port             = kServerPort;
        rest.majordomoTimeout = 800ms; // set timeout to unit-test friendly interval
        auto bound            = broker.bindRest(rest);
        if (!bound) {
            FAIL(fmt::format("Failed to bind REST server: {}", bound.error()));
            return;
        ...
        command.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/clock", kServerPort));
...
TEST_CASE("Majordomo timeouts", "[majordomo][majordomoworker][rest]") {
    ...
    SECTION("Waiting for notification that happens in time gives expected response") {
        ...
        command.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/waiter", kServerPort));
        command.callback = [&responseCount](const auto &msg) {
            REQUIRE(msg.command == mdp::Command::Notify);
            REQUIRE(msg.error == "");
            REQUIRE(msg.data.asString().contains("This is a notification"));
            responseCount++;
        };
        client.request(std::move(command));
    ...
    SECTION("Client handles timeout internally and clients still receive long poll responses eventually") {
        std::atomic<int>               responseCount = 0;
        opencmw::client::RestClient    client;
        opencmw::client::Command       command;
        command.command  = mdp::Command::Subscribe;
    ...
    SECTION("Response to request takes too long, timeout error is returned") {
        std::atomic<int>               responseCount = 0;
        opencmw::client::RestClient    client;
        opencmw::client::Command       command;
        command.command  = mdp::Command::Get;
        command.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/waiter?contentType=application%2Fjson&timeoutMs=1200", kServerPort));
        command.data     = opencmw::IoBuffer("{\"value\": \"Hello!\"}");
        command.callback = [&responseCount](const auto &msg) {
            REQUIRE(msg.command == mdp::Command::Final);
            REQUIRE(msg.data.asString() == "");
            REQUIRE(msg.error == "Timeout");
            responseCount++;
        };
        client.request(std::move(command));
        waitFor(responseCount, 1);
    ...
    SECTION("Response to request arrives in time") {
        ...
        opencmw::client::RestClient  client;
        opencmw::client::Command     command;
        command.command  = mdp::Command::Set;
        command.topic    = opencmw::URI<>(fmt::format("http://localhost:{}/waiter?contentType=application%2Fjson&timeoutMs=0", kServerPort));
        command.data     = opencmw::IoBuffer("{\"value\": \"Hello!\"}");
        command.callback = [&responseCount](const auto &msg) {
            REQUIRE(msg.command == mdp::Command::Final);
            REQUIRE(msg.error == "");
            REQUIRE(msg.data.asString().contains("You said: Hello!"));
            responseCount++;
        };
        client.request(std::move(command));
        waitFor(responseCount, 1);
