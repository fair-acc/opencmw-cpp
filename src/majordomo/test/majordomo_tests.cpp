...
TEST_CASE("NOTIFY example using the BasicMdpWorker class", "[worker][notify_basic_worker]") {
    ...
    while (!seenNotification) {
        worker.notify("/beer.time", "Have a beer");
        {
    ...
    worker.notifyError("/beer.error", "Fridge empty!");
    ...
    worker.notify("/wine.italian", "Try our Chianti!");
    ...
    while (true) {
        worker.notify("/wine.portuguese", "New Vinho Verde arrived.");
        worker.notify("/beer.offer", "Get our pilsner now!");
        worker.notify("/wine.portuguese", "New Vinho Verde arrived.");

        const auto msg1 = client.readOne();
        debug() << "msg1" << msg1.sourceId() << msg1.body();
        REQUIRE(msg1.sourceId() == "/wine*");

        const auto msg2 = client.readOne();
        debug() << "msg2" << msg2.sourceId() << msg2.body();
