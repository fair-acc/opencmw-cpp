...
template<size_t RbSize>
std::vector<std::shared_ptr<Publisher<RbSize>>> makePublishers(size_t size,
        const std::shared_ptr<RingBuffer<TestEvent, RbSize>>         &buffer,
        int                                                   messageCount) {
    std::vector<std::shared_ptr<Publisher<RbSize>>> result;

    for (auto i = 0u; i < size; i++) {
        result.push_back(std::make_shared<Publisher<RbSize>>(buffer, messageCount));
    }
