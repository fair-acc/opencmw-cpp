...
public:
    ...
    SettingBase()
        : SettingBase([](const U & /*old*/, T &&in) -> T { return static_cast<U>(std::move(in)); }){};
    template<class Fn>
...
private:
    [[nodiscard]] std::pair<TimingCtx, const Node &> get(const auto &settingsMap, const TimingCtx &timingCtx) const noexcept {
        for (const auto &[key, value] : settingsMap) {
            if (key.matches(timingCtx)) {
                value.touch();
                return { timingCtx, settingsMap.at(timingCtx) };
            }
        }
        ...
        for (const auto &[key, value] : settingsMap) {
            if (key.matches(withoutGID)) {
                value.touch();
                return { withoutGID, settingsMap.at(withoutGID) };
            }
        }
        const TimingCtx withoutPID(timingCtx.cid(), timingCtx.sid(), -1, -1, bpcts); // eliminate the pid & gid variability
        for (const auto &[key, value] : settingsMap) {
            if (key.matches(withoutPID)) {
                value.touch();
                return { withoutPID, settingsMap.at(withoutPID) };
            }
        }
        const TimingCtx withoutSID(timingCtx.cid(), -1, -1, -1, bpcts); // eliminate the sid & pid & gid variability
        for (const auto &[key, value] : settingsMap) {
            if (key.matches(withoutSID)) {
                value.touch();
                return { withoutSID, settingsMap.at(withoutSID) };
            }
        }
        // did not find an exact match the setting for the specific timing context
